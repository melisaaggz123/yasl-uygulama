...
using System.IO;
using cpark;
using OYUN.Tanıtımlar;

namespace OYUN
{
    
    public partial class Trafikoyun : Form
    {
        Trafiktanıtım trafiktanıtım = new Trafiktanıtım();
        park p1;
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            //for (int i=0; i<1000; i++)
            {
                ...
                //label1.Text = "Çözüm Arıyor ... (Denendi " + trials.ToString() + " Maksimum Çözüm = " + max_sol_steps.ToString() + ")";
                ...
                if (steps_to_sol >= MIN_SOLS)
                {
                    ...
                    res_file.WriteLine (steps_to_sol.ToString ("00 Çözüm: ") +  p.ToString ());
           ...
           // this.Text  = DateTime.Now.Subtract(st).TotalSeconds.ToString();
            ...
            //button3.Enabled = true;//yeni hata verdi

            //button2.Enabled = false;//yeni hata verdi
        ...
        /*private void button4_Click(object sender, EventArgs e)
        {
            ...
            label1.Text = "Çalışıyor ..."; Application.DoEvents();
        ...
        private void button7_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Panodaki Veriler Geçerli Bir Çözüm Bulamıyor!","Hata",MessageBoxButtons.OK ,MessageBoxIcon.Exclamation   );
        ...
        {
            // 1 numaralı aracın en sağda olup olmadığını kontrol et
            int carIndex = 0; // 1 numaralı araç, p1.alignment[0] olduğunda (0 indeksli)
            int carPos = p1.pos[carIndex]; // Aracın başlangıç pozisyonu
            int carLength = p1.length[carIndex]; // Aracın uzunluğu

            // Eğer aracın hizalanması yataysa, son pozisyonu yatay olarak hesapla
            // Eğer dikeyse, son pozisyonu dikey olarak hesapla
            int carEndPos;
            if (p1.alignment[carIndex] == 0) // Yatay
            {
                carEndPos = carPos + carLength - 1;
            }
            else // Dikey
            {
                carEndPos = carPos + (carLength - 1) * p1.width;
            }

            // Aracın son pozisyonunun parkın en sağındaki satıra denk gelip gelmediğini kontrol et
            if (carEndPos == 17) // Aracın son pozisyonu parkın en sağında mı?
            {
                // Eğer araç en sağda ise
                MessageBox.Show("Tebrikler, Kazandınız!", "Başarı");
                return; // Çözüm bulunmuş, işlemi sonlandır
            }
            ...
            if (r.solutions.Count == 0)
            {
                MessageBox.Show("Çözüm Bulunamadı!", "Hata");
        ...
        private void button10_Click(object sender, EventArgs e)
        {
            ...
            //label1.Text = "Çalısıyor ..."; Application.DoEvents();
